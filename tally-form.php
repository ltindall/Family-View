<?php
	...
	if(isset($_SESSION['approving_user_id'])){
		$lastProcess = db_select("SELECT * FROM `tallyTimes` WHERE `type` = 'Processing' ORDER BY `id` DESC LIMIT 1");
		$familyEntries = db_select("SELECT * FROM `service` WHERE `familyID` = ".$_SESSION['approving_user_id']." AND `created` <  '".$lastProcess[0]['timestamp']."'  ORDER BY `servicedate` DESC");
	...
	header('Location: http://'.$_SERVER['HTTP_HOST'].'/home/service-tally.php');
?>
